...
const CIFAR100_RECORD_SIZE =
    CIFAR100_COARSE_LABEL_BYTES +
    CIFAR100_FINE_LABEL_BYTES +
    CIFAR100_IMAGE_SIZE;
...
try {
    ...
    if (fineLabelNames.length !== CIFAR100_NUM_FINE_CLASSES && fineLabelNames.length > 0) {
        console.warn(
        ...
        fineLabelNames = Array.from({ length: CIFAR100_NUM_FINE_CLASSES }, (_, i) => `Class ${i}`);
    }
    console.log(`Loaded ${fineLabelNames.length} fine label names.`);

    console.log("Loading CIFAR-100 training data...");
    const trainDataset = loadCifar100File(TRAIN_FILE_PATH, CIFAR100_NUM_FINE_CLASSES);
    ...
    const testDataset = loadCifar100File(TEST_FILE_PATH, CIFAR100_NUM_FINE_CLASSES);
...
if (testData.length > 0 && testLabels.length > 0) {
    ...
    if (numSamplesToCheck > 0) {
        ...
        for (let i = 0; i < numSamplesToCheck; i++) {
            ...
            displayImageInTerminal(testData[i], CIFAR100_IMAGE_WIDTH, CIFAR100_IMAGE_HEIGHT);
        }
    }
} else {
