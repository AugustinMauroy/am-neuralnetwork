...
const CIFAR_IMAGE_SIZE = CIFAR_IMAGE_HEIGHT * CIFAR_IMAGE_WIDTH * CIFAR_IMAGE_CHANNELS;
...
function loadIdxImages(
    ...
    for (let i = 0; i < numItemsToProcess; i++) { // Use numItemsToProcess instead of numItemsInHeader
...
console.log("Note: MLPs have limitations for image data. CNNs are preferred for better accuracy.");
console.log("This example might be slow and memory-intensive due to the dataset size and MLP structure.");
...
try {
    ...
    trainingData = loadIdxImages(trainImagesPath, CIFAR_IMAGE_HEIGHT, CIFAR_IMAGE_WIDTH, CIFAR_IMAGE_CHANNELS);
    ...
    testData = loadIdxImages(testImagesPath, CIFAR_IMAGE_HEIGHT, CIFAR_IMAGE_WIDTH, CIFAR_IMAGE_CHANNELS);
    ...
    console.error("Error loading CIFAR-10 data. Make sure the .idx*-ubyte files are in the 'example/cifar-10/' directory.", error);
    process.exit(1);
}
...
console.warn(`Using a subset of data: ${subsetSizeTrain} for training, ${subsetSizeTest} for testing.`);
...
if (testData.length > 0 && testLabels.length > 0) {
    ...
    try {
        ...
        console.log("Model Evaluation (conceptual, built-in accuracy is for binary):", evaluation);

        // Manual accuracy calculation for multi-class
        const predictions = model.predict(testData);
        let correctPredictions = 0;
        for (let i = 0; i < predictions.length; i++) {
            const predictedClassIndex = predictions[i].indexOf(Math.max(...predictions[i]));
        ...
        console.log(`Manual Multi-class Accuracy on Test Set: ${(manualAccuracy * 100).toFixed(2)}%`);
    } catch (e) {
...
console.log("Reminder: For better performance on CIFAR-10, consider using Convolutional Neural Networks (CNNs).");
