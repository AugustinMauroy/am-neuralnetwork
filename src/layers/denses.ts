...
export class Dense implements TrainableLayer {
  ...
  private initializeWeights(): number[][] {
    ...
    for (let i = 0; i < this.inputUnits; i++) {
      weights[i] = Array(this.outputUnits).fill(0).map(() => Math.random() * 0.1 - 0.05); // Small random numbers
    }
    return weights;
  }
  ...
  private initializeBiases(): number[] {
    return Array(this.outputUnits).fill(0).map(() => Math.random() * 0.1 - 0.05); // Small random numbers
  }
  ...
  public forward(input: number[][]): number[][] {
    ...
    for (let b = 0; b < batchSize; b++) { // For each sample in the batch
      ...
      for (let j = 0; j < this.outputUnits; j++) { // For each output neuron
        let weightedSum = this.biases[j];
        for (let i = 0; i < this.inputUnits; i++) { // For each input feature
  ...
  public getWeightGradients(
    ...
    if (batchSize === 0) {
      return new Map([
        ["weights", Array(this.inputUnits).fill(null).map(() => Array(this.outputUnits).fill(0))],
    ...
    const dLdW: number[][] = Array(this.inputUnits).fill(null).map(() =>
      Array(this.outputUnits).fill(0)
    );
    const dLdB: number[] = Array(this.outputUnits).fill(0);

    // Accumulate gradients over the batch
    for (let b = 0; b < batchSize; b++) {
      for (let j = 0; j < this.outputUnits; j++) { // For each output unit
        dLdB[j] += outputGradient[b][j]; // Gradient for bias_j
        for (let i = 0; i < this.inputUnits; i++) { // For each input unit
   ...
   */
  public updateWeights(updatedWeights: Map<string, number[] | number[][]>): void {
    const newWeights = updatedWeights.get("weights") as number[][];
    const newBiases = updatedWeights.get("biases") as number[];

    if (
      newWeights && newWeights.length === this.inputUnits &&
      newWeights[0].length === this.outputUnits
    ) {
      this.weights = newWeights;
    } else {
      console.warn("Dense layer: Mismatch in new weights dimensions or new weights not provided.");
    }

    if (newBiases && newBiases.length === this.outputUnits) {
      this.biases = newBiases;
    } else {
      console.warn("Dense layer: Mismatch in new biases dimensions or new biases not provided.");
    }
  }
}
